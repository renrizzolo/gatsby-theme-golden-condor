...
By default `page` and `post` collections are set up, but you can add your own. See @todo<Link to="/doc/collections">collections</Link> for more details.
...
```js
{
  collections: {
    [collectionName]: {
      ...
      // disable the /[collectionName] index page
